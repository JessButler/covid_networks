...
#input file name
input_file <- 
  "cases_contacts_20201001.csv"

#input contact data into dataframe and tidy
contacts_data <- 
  read_delim(input_file, delim = ",")  #change delimiter type here

#check that contacts_data has four necessary columns: case, contact, contact_type and date
columns <- c("case","contact","contact_type","date")

if(!all(i <- has_name(contacts_data, columns)))
    stop(sprintf(
      "The input file is missing column(s) called %s",
      paste(columns[!i], collapse = ", ")))

#check that all variables in the case column start with "CS" 
if (!all(startsWith(contacts_data$case,"CS")))
  stop(sprintf(
    "The names of all cases should start with CS"))

#tidy contacts_data
#create only two categories on contacts: household & non-household
#flag as case if name includes "CS
contacts_data <-
  contacts_data %>%
  mutate(
...
#One of the network components is "membership", a named numeric list where the names are the cases/contacts and the numbers are the groups
members <- 
  sort(network_components$membership) #sort by group

#convert network id and members to dataframe and tidy names
network_members <-
  data_frame(
    members,
    names(members)) %>%
  rename(network_id = members,
         id = "names(members)")
...
table_networks_membership <-
  network_members %>%
  rename(earliest_case_in_network = earliest_case) %>%
  select(id, earliest_case_in_network)
```
...
write_csv(table_networks_membership, 
          here::here("outputs", paste(max(contacts_data$date), "networks_membership.csv", sep = "_")))
```
