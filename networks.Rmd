...
#check that contacts_data contains the four columns we need: case, contact, contact_type and date
...
if (!all(startsWith(contacts_data$case,"CS")))
  stop(sprintf("The names of all Cases in the input file should start with CS"))
...
members <- sort(network_components$membership) #sort by group
network_members <- data.frame(as.integer(members),names(members),row.names = NULL,stringsAsFactors=FALSE) #convert to dataframe
colnames(network_members) <- c("network_id", "id") #fix column names
